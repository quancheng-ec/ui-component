      ...
      <ui-grid-group slot="content">
        <ui-grid-item :space="6">
          ...
          <div class="label-group">
              <span class="label label-rounded label-info label-outline" v-for="chosen,index in chosenList">
                {{chosen.name}}
        ...
        <ui-grid-item :space="6">
          <div class="tree-panel" v-for="item in items">
            <p>{{ labelMap[item] }}：</p>
            <fk-department :department-data="trees[item]" :level="1" :event-bus="eventBus" :url="url"
                           :account-only="item === 'account'" :need-account="item === 'account'"></fk-department>
          </div>
        </ui-grid-item>
      </ui-grid-group>
      <div slot="foot-btn">
        <ui-button type="info" @click.native="closeModal('save')">保存</ui-button>
        <ui-button type="info outline" @click.native="closeModal('cancel')">取消</ui-button>
      </div>
    </ui-modal>
  </div>
</template>
